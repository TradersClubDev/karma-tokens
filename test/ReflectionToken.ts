...
import { ReflectionToken, Token } from "../typechain-types";
import {
  IUniswapV2Router02,
  UniswapV2Deployer,
...
describe("ReflectionToken", function () {
  ...
  let tokenContract: ReflectionToken;
  let rewardToken: Token;

  this.beforeAll(async () => {
    const tokenFactory = await ethers.getContractFactory("ReflectionToken");
    tokenContract = (await tokenFactory.deploy()) as ReflectionToken;
    await tokenContract.deployed();

    const rewardTokenFactory = await ethers.getContractFactory("Token");
    rewardToken = (await rewardTokenFactory.deploy()) as Token;
  ...
  describe("Deployment", function () {
    it("Initialized", async function () {
      ...
      const tokenDataReward = {
        ...
        buyTax: { marketing: 50n, reflection: 50n },
        sellTax: { marketing: 100n, reflection: 50n },
      ...
      expect(await rewardToken.initialize(tokenDataReward)).to.not.revertedWithPanic();
    ...
    it("buy", async () => {
      ...
      expect(await tokenContract.balanceOf(user2.address)).to.eq(8676128109405831288049n);
    });
