...
contract StandardToken is BaseToken {
    ...
    function swapForFees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 devAmt = unitBalance * 2 * sellTaxes.devOrReflection;
            if (devAmt > 0) {
    ...
    function updateMaxTxAmount(uint256 amount) external onlyOwner {
        require(amount > _totalSupply / 1000, 'maxTxAmount < 0.1%');
        maxTxAmount = amount;
    }

    function updateMaxWalletAmount(uint256 amount) external onlyOwner {
        require(amount > _totalSupply / 1000, 'maxWalletAmount < 0.1%');
    ...
    function manualSwap(uint256 amount, uint256 devPercentage, uint256 marketingPercentage) external onlyOwner {
        ...
        if (devPercentage > 0 && devWallet != address(0x0))
